...
static u8_t generate_keys(void)
{
    ...
    do {
        do {
            bt_rand(ecc.private_key, 32);
        } while (!P256_ecdh_keygen(ecc.pk, ecc.private_key));
...
static void emulate_le_p256_public_key_cmd(void)
{
    ...
    if (status) {
         ...
         */
        memcpy(evt->key, ecc.pk, 32);
        memcpy(&evt->key[32], &ecc.pk[32], 32);
...
static void emulate_le_generate_dhkey(void)
{
    ...
    if (!P256_ecdh_shared_secret(ecc.dhkey, ecc.pk, ecc.private_key)) {
        ret = TC_CRYPTO_FAIL;
    } else {
...
void clear_ecc_events(struct net_buf *buf)
{
...
void le_gen_dhkey(struct net_buf *buf)
{
     ...
     */
    memcpy(ecc.pk, cmd->key, 32);
    memcpy(&ecc.pk[32], &cmd->key[32], 32);
    k_sem_give(&cmd_sem);
...
void le_p256_pub_key(struct net_buf *buf)
{
    u8_t status;
    net_buf_unref(buf);
...
void bt_hci_ecc_init(void)
{
    ...
    k_thread_create(&ecc_thread_data, ecc_thread_stack,
                    ...
                    NULL, NULL, NULL, 41 - 26, 0, K_NO_WAIT);
}
