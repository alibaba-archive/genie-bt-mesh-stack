...
.set use_mul_for_sqr, 0 // 1 to enable, 0 to disable (14% slower if enabled but saves 624/460 bytes (depending on use_noninlined_sqr64) of compiled code size)

	
.set use_noninlined_mul64, 1 // 1 to enable, 0 to disable (2.6%/4.0% slower (depending on use_mul_for_sqr) if enabled but saves 308 bytes of compiled code size)

	
.set use_noninlined_sqr64, 1 // 1 to enable, 0 to disable (2.4% slower if enabled and use_mul_for_sqr=0 but saves 164 bytes of compiled code size)

	
.set use_interpreter, 1 // 1 to enable, 0 to disable (3.6% slower if enabled but saves 268 bytes of compiled code size)

	
.set use_smaller_modinv, 0 // 1 to enable, 0 to disable (3.6% slower if enabled but saves 88 bytes of compiled code size)
...
P256_sqr64:
 ...
 // Leaves r6 zero.
	bx lr
	.size P256_sqr64, .-P256_sqr64

	.type P256_sqr128, %function
P256_sqr128:
	push {lr}
	//frame push {lr}
...
// output: 1 if value >= limit, otherwise 0
	.type P256_greater_or_equal_than, %function
P256_greater_or_equal_than:
	push {r4-r6,lr}
	//frame push {r4-r6,lr}
...
0:
	...
	mov r0,r5
	pop {r4-r6,pc}
	.size P256_greater_or_equal_than, .-P256_greater_or_equal_than
...
P256_div2mod:
	...
	stm r0!,{r4-r7}
	bx lr
	.size P256_div2mod, .-P256_div2mod
...
// for a function taking less than two parameters, the extra parameters are ignored
	.type P256_interpreter, %function
P256_interpreter:
	push {r4-r7,lr}
	//frame push {r4-r7,lr}
...
1:
	...
	//frame address sp,20
	pop {r4-r7,pc}
	.size P256_interpreter, .-P256_interpreter
