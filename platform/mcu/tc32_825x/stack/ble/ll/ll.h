...
#if 1	// fix for aos compile error.
...
void  smemset(register char * dest,register int val,register unsigned int len);
void smemcpy(register char * out, register char * in, register unsigned int len);
...
typedef struct {
	...
	u8	cur_rx_phy;
	u8	pre_all_phys;
	u8	pre_tx_phys;	//phy preferred
	u8	pre_rx_phys;
}ll_phy_t;
extern ll_phy_t bltPhy;


ble_sts_t	blc_ll_readPhy(u16 connHandle, hci_le_readPhyCmd_retParam_t *);
ble_sts_t 	blc_ll_setDefaultPhy(u8 all_phys, u8 tx_phys, u8 rx_phys);
ble_sts_t  	blc_ll_setPhy(u16 connHandle, u8 all_phys, u8 tx_phys, u8 rx_phys, u16 phy_options);
